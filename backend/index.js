require('dotenv').config(); // Load environment variables from .env file
const dns = require('dns');

// Force Node to use Google DNS
dns.setServers(['8.8.8.8', '8.8.4.4']);
const express = require('express');
const mongoose = require('mongoose');
const cors = require('cors');
const bodyParser = require('body-parser');

// bodyParser is built into express now; no need to import twice


const {HoldingsModel}=require('./model/HoldingsModel'); // Import the Holdings model
const {PositionsModel}=require('./model/PositionsModel'); // Import the Positions model
const {OrdersModel}=require('./model/OrdersModel'); // Import the Orders model

const PORT=process.env.PORT || 5000;
const uri=process.env.MONGO_URL;

const app=express(); // Create an instance of the Express application

const cookieParser = require("cookie-parser");
const authRoute = require("./Routes/AuthRoute");

// middleware
app.use(express.json());
app.use(cors(
    {
    origin: [
        "http://localhost:5173", 
        "http://localhost:5174",
        "https://your-frontend.vercel.app",
        "https://your-dashboard.vercel.app"    
    ], // Allow Vite frontend
    methods: ["GET", "POST", "PUT", "DELETE"],
    credentials: true, // Required for cookies/sessions
  }
)); // allow cross‑origin requests from the frontend
app.use(cookieParser());

// app.use(cors());  
app.use(bodyParser.json()); // Parse JSON request bodies

// app.get('/addHoldings', async(req, res) => {
//     let tempHoldings=[
//     {
//     name: "BHARTIARTL",
//     qty: 2,
//     avg: 538.05,
//     price: 541.15,
//     net: "+0.58%",
//     day: "+2.99%",
//   },
//   {
//     name: "HDFCBANK",
//     qty: 2,
//     avg: 1383.4,
//     price: 1522.35,
//     net: "+10.04%",
//     day: "+0.11%",
//   },
//   {
//     name: "HINDUNILVR",
//     qty: 1,
//     avg: 2335.85,
//     price: 2417.4,
//     net: "+3.49%",
//     day: "+0.21%",
//   },
//   {
//     name: "INFY",
//     qty: 1,
//     avg: 1350.5,
//     price: 1555.45,
//     net: "+15.18%",
//     day: "-1.60%",
//     isLoss: true,
//   },
//   {
//     name: "ITC",
//     qty: 5,
//     avg: 202.0,
//     price: 207.9,
//     net: "+2.92%",
//     day: "+0.80%",
//   },
//   {
//     name: "KPITTECH",
//     qty: 5,
//     avg: 250.3,
//     price: 266.45,
//     net: "+6.45%",
//     day: "+3.54%",
//   },
//   {
//     name: "M&M",
//     qty: 2,
//     avg: 809.9,
//     price: 779.8,
//     net: "-3.72%",
//     day: "-0.01%",
//     isLoss: true,
//   },
//   {
//     name: "RELIANCE",
//     qty: 1,
//     avg: 2193.7,
//     price: 2112.4,
//     net: "-3.71%",
//     day: "+1.44%",
//   },
//   {
//     name: "SBIN",
//     qty: 4,
//     avg: 324.35,
//     price: 430.2,
//     net: "+32.63%",
//     day: "-0.34%",
//     isLoss: true,
//   },
//   {
//     name: "SGBMAY29",
//     qty: 2,
//     avg: 4727.0,
//     price: 4719.0,
//     net: "-0.17%",
//     day: "+0.15%",
//   },
//   {
//     name: "TATAPOWER",
//     qty: 5,
//     avg: 104.2,
//     price: 124.15,
//     net: "+19.15%",
//     day: "-0.24%",
//     isLoss: true,
//   },
//   {
//     name: "TCS",
//     qty: 1,
//     avg: 3041.7,
//     price: 3194.8,
//     net: "+5.03%",
//     day: "-0.25%",
//     isLoss: true,
//   },
//   {
//     name: "WIPRO",
//     qty: 4,
//     avg: 489.3,
//     price: 577.75,
//     net: "+18.08%",
//     day: "+0.32%",
//   },
//     ];
//     tempHoldings.forEach((item)=>{
//         const newHolding=new HoldingsModel({
//             name: item.name,
//             qty: item.qty,
//             avg: item.avg,
//             price: item.price,
//             net: item.net,
//             day: item.day,
//         });
//         newHolding.save();
//     });
//     res.send("Holdings added to the database");
// });

// app.listen(PORT, () => {
//     console.log(`Server running on port ${PORT}`);
//     mongoose.connect(uri);
//     console.log('Connected to MongoDB');
//     // .then(() => console.log('Connected to MongoDB'))
//     // .catch((err) => console.error('Error connecting to MongoDB:', err));
// });

// app.get('/addPositions', async(req, res) => {
//     let tempPositions=[
//     {
//     product: "CNC",
//     name: "EVEREADY",
//     qty: 2,
//     avg: 316.27,
//     price: 312.35,
//     net: "+0.58%",
//     day: "-1.24%",
//     isLoss: true,
//   },
//   {
//     product: "CNC",
//     name: "JUBLFOOD",
//     qty: 1,
//     avg: 3124.75,
//     price: 3082.65,
//     net: "+10.04%",
//     day: "-1.35%",
//     isLoss: true,
//   },
//     ];
//     tempPositions.forEach((item)=>{
//         let newPosition=new PositionsModel({
//             product: item.product,
//             name: item.name,
//             qty: item.qty,
//             avg: item.avg,
//             price: item.price,
//             net: item.net,
//             day: item.day,
//             isLoss: item.isLoss,
//         });
//         newPosition.save();
//     });
//     res.send("Positions added to the database");
// });

app.use("/", authRoute);

app.get('/allHoldings', async(req, res) => {
    let allHoldings = await HoldingsModel.find({}); 
    res.json(allHoldings);
});
app.get('/allPositions', async(req, res) => {
    let allPositions=await PositionsModel.find({});
    res.json(allPositions);
});
// app.post('/newOrder', async (req, res) => {
//     // log request body for debugging
//     console.log('POST /newOrder body:', req.body);

//     // coerce incoming values to numbers to avoid string concatenation
//     const name = req.body.name;
//     const qty = Number(req.body.qty);
//     const price = Number(req.body.price);
//     const mode = req.body.mode;

//     let newOrder = new OrdersModel({
//         name,
//         qty,
//         price,
//         mode,
//     });
//     await newOrder.save();
//     res.send('Order saved!');
// });

app.post('/newOrder', async (req, res) => {
    const { name, mode } = req.body;
    const qty = Number(req.body.qty);
    const price = Number(req.body.price);

    try {
        // 1. Create the Order Record (History)
        let newOrder = new OrdersModel({ name, qty, price, mode });
        await newOrder.save();

        // 2. Update Holdings based on Mode
        if (mode === "BUY") {
            // If exists, increment qty. If not, create new holding (upsert).
            await HoldingsModel.findOneAndUpdate(
                { name: name },
                { 
                    $inc: { qty: qty }, 
                    $set: { price: price, avg: price, net: "0%", day: "0%" } 
                },
                { upsert: true, returnDocument: 'after'}
            );
        } 
        else if (mode === "SELL") {
            const holding = await HoldingsModel.findOne({ name: name });

            if (!holding || holding.qty < qty) {
                return res.status(400).send('Insufficient quantity to sell');
            }

            const updatedQty = holding.qty - qty;

            if (updatedQty === 0) {
                await HoldingsModel.deleteOne({ name: name });
            } else {
                await HoldingsModel.updateOne(
                    { name: name },
                    { $set: { qty: updatedQty } }
                );
            }
        }

        res.send('Order processed and holdings updated!');
    } catch (err) {
        console.error(err);
        res.status(500).send('Internal Server Error');
    }
});


app.listen(PORT, async () => {
    console.log(`Server running on port ${PORT}`);
    
    try {
        await mongoose.connect(uri, {
            family: 4, // Force IPv4 to avoid DNS resolution issues
        });
        console.log('Connected to MongoDB successfully!');
        
        // Initialize holdings on startup
        // await initializeHoldings();
    } catch (err) {
        console.error('CRITICAL: Could not connect to MongoDB.');
        console.error('Error Details:', err.message);
        
        if (err.message.includes('ENOTFOUND')) {
            console.log('--- ACTION REQUIRED ---');
            console.log('Your network/ISP is blocking the MongoDB address.');
            console.log('Try switching to a Mobile Hotspot to test.');
        }
    }
});

